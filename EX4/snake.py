...
class Game:
    ...
    def create_board(self):
        ...
        # put obstacles
        while np.sum(
                self.board == OBSTACLE_VAL) < self.args.board_h * self.args.board_w * self.args.obstacle_density:
    ...
    def put_snake_on_board(self, pid):
        self.board[
            self.players[pid].chain_as_np_array()[:, 0], self.players[pid].chain_as_np_array()[:, 1]] = pid
    ...
    def render_board(self, board_np, timestep):
        ...
        for timestep in range(board_np.shape[0]):
            ...
            board.append(
                '|' + ''.join(self.render_map[board_np[timestep, c]] for c in range(board_np.shape[1])) + '|')
    ...
    def render(self, timestep):
        ...
        for timestep in range(self.board.shape[0]):
            # board.append('|' +''.join(self.render_map[self.board[timestep, c]] for c in range(self.board.shape[1])) + '|')
            board.append('|' + ''.join(
                self.render_map[self.board[timestep, c]] for c in range(self.board.shape[1])) + '|')
        board.append(horzline)
        print('\n'.join(board))
